...
def scan_roots(roots: list[Path]) -> GraphExtraction:
    ...
    for root in roots:
        ...
        for py in _py_files(r):
            rel = str(py.relative_to(_repo_root())) if py.is_relative_to(_repo_root()) else str(py)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="输出 .md 路径（默认: log/langgraph.md）",
    )
    args = ap.parse_args()
    repo = _repo_root()
    roots = [repo / "src"] if args.roots is None else list(args.roots)
    out_path = args.output or (repo / "log" / "langgraph.md")
