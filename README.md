...
全局配置在仓库根 `settings.json`（首次从 `template/settings.json` 复制；勿提交含密钥副本）。与模板**深度合并**，本地可只写需覆盖的字段；顶层分组为 `llm`、`planning`、`context`、`feishu` 等。
...
**规划模式现状（简要）**：每条用户消息触发一次完整规划图运行；在单条消息内，模型按 `thought →（可选）action → observation → next_step` 迭代，每轮 `thought` 都会把全量对话与本轮 Planner 系统提示一并送入模型，步数受 `max_cycles` 约束（默认见 `planning.default_max_cycles`，可用 `planning.max_cycles` 或 stdio 的 `set_plan_cycles` 调整，上限见 `planning.max_cycles_cap`），达到上限则进入无工具的 `finalize` 强制收尾。`cycle_count`、`task_outline` 等仅存在于当次图运行，不跨用户轮次保留；更长流程需提高上限、拆成多条消息，或依赖对话历史与 `memory/`、`/compass` 控制 token。整体是**有界 ReAct**，而非无限工具循环。
...
**单条用户消息**内的工具链长度受 `max_cycles` 约束（默认与上限见 `settings.json` 中 `planning.default_max_cycles`、`planning.max_cycles_cap`，会话内可用 `set_plan_cycles` 覆盖）。需要更长链路时可提高该值，或把任务拆成多条用户消息（每条消息会重新跑一轮规划图；跨条的进度靠对话历史与 `memory/` 衔接）。上下文 token 压力仍受 `token_budget` 约束，长会话可配合 CLI/TUI 的 `/compass` 与会话摘要，避免「步数放开」后把整段历史原样撑满窗口。`planning_trace` 记录节点级摘要，便于导出图或对照日志。`remember` 在循环推进后把「目标—子目标—观察—结果」式条目写入规划长期记忆文件。设计意图是可解释、有上限、对失败敏感，而不是无界 ReAct。
